def loc_min_max_stg(num_rows, dollars, adj_closing_val_list, assumed_trade_units):
    ...
    for i in range(num_rows-2):
        ...
        if (slope_b > 5.0 and slope_a <= 5.0) and trade_d_per_index >= max_trade:  #If local min and there are enough dollars for trade, buy
            ...
            tot_num_trades += 1
        elif (slope_b < -5.0 and slope_a >= -5.0) and val_per_index > max_trade:  #If local max and there are enough invested dollars, sell
            trade_d_per_index += max_trade
            trade_dollars = trade_d_per_index * adj_closing_val_list[i+2]
